...
exports.run = async (client, message, args) => {
    ...
    let data = db.all().filter(i => i.ID.startsWith("noglobalchat_")).sort((a, b) => b.data - a.data)
    if (data.length < 1) return message.inlineReply("Sem ranking por enquanto").then(m => m.delete({ timeout: 5000 }))

    let myrank = data.map(m => m.ID).indexOf(`noglobalchat_${message.author.id}`) + 1 || "N/A"
    data.length = 10
    let lb = []
    for (let i in data) {
